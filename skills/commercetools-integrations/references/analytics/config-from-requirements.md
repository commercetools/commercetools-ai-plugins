...
- **Least-privilege, read-only.** Egress reads commercetools and writes the *destination* — so the commercetools scopes are **read** scopes for the domains you export plus `manage_subscriptions` for the streamer's registration. Never an admin/`manage_project` client. → commercetools-connect [security.md](../../../commercetools-connect/references/security.md).
