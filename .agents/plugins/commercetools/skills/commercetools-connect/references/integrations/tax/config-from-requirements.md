...
description: Map tax requirements to a commercetools tax connector's connect.yaml — tax mode (ExternalAmount vs External), nexus, tax-code source, exemptions, least-privilege scopes — with a worked example. The tax sub-area of commercetools-connect.
metadata:
