...
A partner-private connector can't be forked: a genuine gap there means working with the vendor or going to path 3. The fork's build/stage/publish lifecycle is the parent [commercetools-connect](../../../SKILL.md) skill.
...
- the parent skill's [production-readiness checklist](../../../SKILL.md#production-readiness-checklist-the-gate) — inbound authentication, native client provisioning, secrets in `securedConfiguration`, no stack traces in responses, idempotent lifecycle scripts, structured logs, health endpoint, tests that assert behavior, README;
