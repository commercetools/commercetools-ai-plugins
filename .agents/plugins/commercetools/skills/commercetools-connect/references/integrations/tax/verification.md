...
description: Verify a tax connector round trip — taxedPrice on the cart, transaction recorded in the engine — and the two traps that look like bugs but aren't (sandbox doesn't persist; no nexus means zero tax). The tax sub-area of commercetools-connect.
metadata:
