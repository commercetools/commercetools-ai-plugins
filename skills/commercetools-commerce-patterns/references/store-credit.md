...
2. **Create the Payment Method:** Generate a `PaymentMethodDraft` to represent the Store Credit. Set the `method` to "StoreCredit" and include the tokenized account reference.
...
With the introduction of the native **Payment Methods API** (currently in Public Beta), our recommended approach is to utilize this resource for storing and managing Store Credit metadata as described in Approach 3.

This API provides the most semantically correct and secure method for managing payment instruments within **commercetools Composable Commerce**. Furthermore, adopting this native pattern ensures forward compatibility with evolving platform features such as **ctCheckout** and **recurring order** workflows.

If your organization's risk profile requires avoiding features currently in Public Beta, we recommend defaulting to **Approach 2: Leveraging Custom Objects**. This alternative provides a similar level of operational security by keeping credit data read-only within the Merchant Center while maintaining a clean separation from core Customer profile data.
