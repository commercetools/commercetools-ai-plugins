...
Each sub-area lives under [`references/integrations/<type>/`](./references/integrations/) with its own `overview.md`. Adding another connector type later (e.g. shipping) means adding a sibling `references/integrations/<type>/` tree and one row here — the build-side guidance does not change.
