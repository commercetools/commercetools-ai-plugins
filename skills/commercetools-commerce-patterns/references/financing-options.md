...
**A:** There are two ways to do it, none of them perfect and each has its own pros and cons. When the question was asked the customer wanted to use product discounts — this option is not good for two reasons: 1) only one product discount is applied and 2) there is no support for custom fields to add the necessary information. The two options we see are:
...
**Option 1 alternative:** Could be implemented using product discounts with custom fields — there is a PB entry to add custom fields to product discounts:
- https://commercetools.productboard.com/insights/notes/view/1445-all-notes/notes/32533303
