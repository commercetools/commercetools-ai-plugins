...
description: Integrate a gift card management system into commercetools as a Checkout payment method via a Connect connector — the two-app workflow (requirements → use a public connector, customize/fork one, or build a new one → config → balance + redeem + refund). The gift card sub-area of commercetools-integrations.
metadata:
