...
This is the **order-management sub-area** of this skill: you need to connect commercetools to an order-management system (OMS) — export placed Orders downstream and keep status, shipment, fulfillment, and inventory in sync. The build-side platform contracts (event/service/job, idempotency, lifecycle, security) are the [commercetools-connect](../../../commercetools-connect/SKILL.md) skill's; this sub-area owns the OMS-specific decision (**use a public connector, customize one, or build a new one**) and the **sync design** that sits on top of those contracts.
...
3. **A connector exists but has a genuine gap config can't close** → **fork/customize** it (if its source is available) — add only the delta and deploy as an Organization connector. Don't rebuild a working, maintained connector. → [connector-selection.md](./connector-selection.md), then the [commercetools-connect](../../../commercetools-connect/SKILL.md) build-side.
...
- **Export** = an `event` application subscribing to `OrderCreated` (and status Messages) → [event-applications.md](../../../commercetools-connect/references/event-applications.md). At-least-once, no ordering: idempotent on `orderNumber`/OMS id, re-fetch the Order by id, filter self-changes.
- **Inbound** = a `service` application as an **inbound webhook** the OMS calls → [service-applications.md](../../../commercetools-connect/references/service-applications.md). Authenticate the caller, validate the payload, apply updates idempotently (upsert by key / re-check state), never blind-create.
- **Reconcile** = a `job` for nightly full sync / drift repair → [job-applications.md](../../../commercetools-connect/references/job-applications.md). Owns its own locking and checkpointing.
- **Registration** of the Subscription and any custom types/statuses = idempotent `postDeploy` / `preUndeploy` → [lifecycle-scripts.md](../../../commercetools-connect/references/lifecycle-scripts.md).

Build **test-first** (commercetools-connect skill's Quality gate): write the failing test that names the behavior, confirm it's red for the right reason, write the least code to pass, refactor. Mock the outbound boundary (the OMS API, the commercetools API) and assert on what your code decided to do. → [testing.md](../../../commercetools-connect/references/testing.md).

### Step 4 — Deploy

Deploy is **type-agnostic** — use the commercetools-connect skill's [deployment-installation.md](../../../commercetools-connect/references/deployment-installation.md). A **public** connector installs directly (Connect CLI `deployment create --connector-key`, or the Connect UI); a **forked/built** Organization connector goes through `connectorstaged create → publish → deployment create`. Pass the config you derived; secrets go in `securedConfiguration`, never in code.
...
| Event app (export): envelope, ack, idempotency, re-fetch, injected destination | [event-applications.md](../../../commercetools-connect/references/event-applications.md) |
| Service app (inbound webhook): authenticated inbound, idempotent upsert, timeout | [service-applications.md](../../../commercetools-connect/references/service-applications.md) |
| Job app (reconcile): schedule, timeout, concurrency, checkpointing | [job-applications.md](../../../commercetools-connect/references/job-applications.md) |
| Idempotent Subscription/custom-type registration in postDeploy/preUndeploy | [lifecycle-scripts.md](../../../commercetools-connect/references/lifecycle-scripts.md) |
| Deploy/install (public vs forked/built), regions, redeploy | [deployment-installation.md](../../../commercetools-connect/references/deployment-installation.md) |
| Testing (auth matrix, idempotency, ack edge cases), test-first loop | [testing.md](../../../commercetools-connect/references/testing.md) |
| Logs + correlation IDs, health, poison-message/replay runbook | [observability-operations.md](../../../commercetools-connect/references/observability-operations.md) |
