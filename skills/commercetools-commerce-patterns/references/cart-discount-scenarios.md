...
**Source:** Cart Discount Scenarios PDF (internal PS reference)
...
**Configurable for:** Item (Amount off only), Buy and Get (Amount off + Fixed price), Discount Bundles (Amount off + Fixed price). Percentage off (relative) discounts apply individually by nature — each item gets the percentage directly.

---

## Recent Enhancement: Proportional Distribution in Buy & Get

Previously, when a Buy & Get discount used a relative (percentage) value, the full discount applied only to the **target item**. This made return handling complex: if the trigger item was returned, the discount amount wasn't easily recalculated.

**New behavior (2026):** Relative Buy & Get discounts can now distribute the discount amount **proportionately across both trigger and target line items** — not just the target. This means:
...
- `categories.key contains "beds"` — matches only items directly assigned to a category with key "beds"
- `categoriesWithAncestors.key "beds"` — matches items in "beds" AND any subcategory of "beds"
...
```json
{
  ...
  "target": {
    ...
    "triggerPattern": [
      {
        "type": "CountOnLineItemUnits",
        "predicate": "categories.key contains \"furniture\" and price.centAmount > 100",
        "minCount": 1,
        "maxCount": 1
      }
    ],
...
Key notes:
- `price.centAmount > 100` inside the trigger predicate filters on per-item price, not cart total. This is a `CountOnLineItemUnits` predicate — it can combine category and price conditions on the line item.
...
```json
{
  ...
  "target": {
    ...
    "targetPattern": [
      {
        "type": "CountOnLineItemUnits",
        "predicate": "categoriesWithAncestors.key \"beds\"",
        "minCount": 1,
        "maxCount": 1
      }
    ],
...
Key notes:
- **`categoriesWithAncestors.key "beds"`** instead of `categories.key contains "beds"` — this is the only way to include products assigned to subcategories of "beds" (e.g., "king-beds", "bunk-beds"). Use `categoriesWithAncestors` whenever the category hierarchy matters.
...
```json
{
  ...
  "target": {
    ...
    "targetPattern": [
      {
        "type": "CountOnLineItemUnits",
        "predicate": "categories.key includes \"candle-holders\"",
        "minCount": 2,
        "maxCount": 2
      }
    ],
...
**Business intent:** When the cart net total reaches $100, add a specific gift SKU to the cart at zero cost.
...
```json
{
  ...
  "target": {
    "type": "lineItems",
    "predicate": "variant.key = \"gift-item-sku\""
  }
}
```

Key notes:
- The gift SKU must exist as a product variant in the catalog and be added to the cart before or alongside the discount. The 100% discount brings its price to zero.
- In the cart response, the gifted line item shows `"lineItemMode": "GiftLineItem"` and `discountedPrice.centAmount: 0`. This field distinguishes gift items from regular purchased items.
...
- For "customer's choice of gift from N items" — use a hidden category containing all eligible gift SKUs and apply 100% off to `categories.key contains "hidden-gift-category"`. The customer selects and adds their preferred item; the discount handles the zero-pricing.
...
| Mode | Behavior |
|------|----------|
| **Best Deal** | Engine compares product discount and cart discount; applies only the one giving the greatest saving to the customer. The cart response includes a `discountTypeCombination` field showing which type won. |
...
- A discount code can be associated with up to **500 stores**
