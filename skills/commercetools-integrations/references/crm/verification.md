...
description: Verify a CRM connector round trip — counterpart record linked by externalId, a delta propagates exactly once, deletion propagates — and the traps that look like bugs but aren't (sync loops, rate-limit throttling, sandbox quirks). The CRM sub-area of commercetools-integrations.
metadata:
