...
Fetch exact fields/actions with this skill's schema scripts before writing code — do not hardcode field lists from memory:
...
React to a placed Order and push it downstream. Build on [event-applications.md](../../../commercetools-connect/references/event-applications.md) — it owns the platform contract; below is only what's OMS-specific.
...
The OMS calls your endpoint when status, shipment, fulfillment, or inventory changes. Build on [service-applications.md](../../../commercetools-connect/references/service-applications.md) as the **inbound-webhook** mode (5-min service timeout, not the 2-s Extension limit; you authenticate the caller and call the commercetools API yourself — no Extension is registered).

- **Authenticate the caller** and validate the payload before touching commercetools ([security.md](../../../commercetools-connect/references/security.md)).
...
A scheduled full/delta sync that repairs drift the event/webhook path missed (dropped webhook, poison message, backfill). Build on [job-applications.md](../../../commercetools-connect/references/job-applications.md): owns its own overlap locking and restart-safe checkpointing; each unit idempotent so a re-run can't double-write. Use it for nightly inventory snapshots and to re-push Orders the OMS never acknowledged.
...
- **No loops.** If both an export subscription *and* an inbound webhook can touch order status, they must not master the same field. When the connector writes to the Order, that write emits its own Message — filter self-changes so the export doesn't re-push what the inbound flow just applied ([event-applications.md](../../../commercetools-connect/references/event-applications.md) self-change filtering).
