...
description: The two-app tax connector contract — the calculator API Extension (ExternalAmount, the four tax update actions, 200-not-202, fail modes, call reduction) and the order-syncer Subscription (commit/void/refund lifecycle, idempotency). Full pitfall catalog. The tax sub-area of commercetools-integrations.
metadata:
