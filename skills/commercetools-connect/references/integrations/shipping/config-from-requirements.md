...
Every value above is a **decision you can defend**; if you can't say why a key exists, drop it. Envelope mechanics and the deploy flow: [deployment-installation.md](../../deployment-installation.md). Idempotent registration of the Extension, Subscription, Types, and fallback Shipping Method: [lifecycle-scripts.md](../../lifecycle-scripts.md).
...
- **The Cart API Extension** — registered with an authenticated destination; use the discriminator value `AuthorizationHeader` (not the schema type name `AuthorizationHeaderAuthentication`, which fails with `InvalidJsonInput`), and confirm registration with `GET /{projectKey}/extensions` afterwards ([security.md](../../security.md)).
