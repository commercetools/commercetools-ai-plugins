...
Consider a scenario where you want to allow customers to pick up certain products in-store while shipping others to their home or a designated delivery address. While the process for managing this use case is documented in the [commercetools tutorial on multiple shipping addresses/methods](https://docs.commercetools.com/tutorials/multiple-shipping-addresses-methods#using-multiple-shipping-methods), this article provides additional context and insights.

## High Level Flow

1. Set the `shippingMode` on the Cart to `Multiple`
2. Add the item shipping addresses (`addItemShippingAddress`)
3. Query shipping methods to determine eligibles
4. Add the selected shipping methods to the cart (`addShippingMethod`)
5. Set line item shipping details (`setLineItemShippingDetails`)
...
* Copy over the line items and discount codes from the original cart to the new one using the `addLineItem` & `addDiscountCode` Update Cart actions.
  ...
  * The redundant cart with `shippingMode` `Single` may be abandoned and would get deleted based on the `lastModifiedAt` datetime value, or you may choose to delete it yourself using the Delete Cart by ID operation.
...
**Important note:** The `shipping-methods/matching-cart` API does **not** support carts with `shippingMode` set to `Multiple`.
