...
This is the **shipping sub-area** of [commercetools-connect](../../../SKILL.md): you need shipping options, prices, labels, or tracking to come from an external carrier or shipping service rather than from static Shipping Method rates. The type-agnostic build contracts (service/event/job, idempotency, lifecycle, security) belong to the parent skill; this sub-area owns the shipping-specific decisions — **whether you need a connector at all**, which path to take, and **how an externally quoted rate actually lands on the Cart**.
...
Mock the carrier boundary and assert on what your code *decided* — which quote it picked, which update action it emitted, what it did when the carrier timed out. The suite must run with no deployment and no secrets ([testing.md](../../testing.md)).
...
- [ ] Parent production-readiness gate satisfied ([commercetools-connect](../../../SKILL.md))
