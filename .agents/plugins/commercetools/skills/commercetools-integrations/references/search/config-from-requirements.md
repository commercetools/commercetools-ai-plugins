...
description: Shape the search document and map search requirements to a connector's connect.yaml — the two apps (full ingestion + incremental updater), index/engine keys, least-privilege scopes, secured config — with a worked example. The search sub-area of commercetools-integrations.
metadata:
