...
- `manage_project` in the scopes → STOP. Tell the user this is an admin client and must not be deployed. They need to create a new **Frontend** API client in commercetools Merchant Center (Settings → Developer settings → API clients → Create new, use the **Frontend B2C** template (or Frontend B2B), then make sure `manage_payments`, `manage_sessions` and `manage_orders` are included).
