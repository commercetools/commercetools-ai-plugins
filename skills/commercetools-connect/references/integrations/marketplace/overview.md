...
>
> "Build a marketplace connector" almost always means the second. If the user actually meant "publish my connector on the Connect marketplace", that's the parent skill's [deployment-installation.md](../../deployment-installation.md), not this file.

**Nothing here belongs on the cart hot path.** Offer sync, order export, and inventory updates are asynchronous by nature — they must never be registered as an API Extension. The one arguable exception is a cross-seller *cart validation* extension (e.g. rejecting a cart that mixes sellers who can't ship together); if the user needs that, price it against the extension timeout budget in [service-applications.md](../../service-applications.md) first, and keep it separate from the sync apps.
