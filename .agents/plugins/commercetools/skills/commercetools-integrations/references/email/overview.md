...
description: Integrate a transactional email service (SendGrid, Mailgun, AWS SES, Postmark, …) into commercetools via a Connect connector — the one-app, event-driven workflow (requirements → is-a-ready-made-connector-enough → config → send + verify). The email sub-area of commercetools-integrations.
metadata:
...
1. **Use a public connector as-is** (rung 1) → install + configure it; the emails are authored and sent inside that platform. Installation (CLI auth, scopes, `deployment create`) is the commercetools-connect skill's [deployment-installation.md](../../../commercetools-connect/references/deployment-installation.md).
...
| Generic event-app contract (envelope, ack table, idempotency, re-fetch) — this sub-area builds on it | [event-applications.md](../../../commercetools-connect/references/event-applications.md) |
