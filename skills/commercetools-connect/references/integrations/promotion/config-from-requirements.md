...
description: Map promotion requirements to a commercetools promotion connector's connect.yaml — how discounts land on the cart (setDirectDiscounts vs custom line items), coupon-code custom field, effect mapping, least-privilege scopes — with a worked example. The promotion sub-area of commercetools-connect.
metadata:
...
The extension destination URL must include the endpoint path (`<url>/promotionEvaluator`), and the Express router must be mounted at the same base path — a mismatch 404s all platform traffic ([project-structure.md](../../project-structure.md)).
...
The custom type creation belongs in `postDeploy`, get-then-update so a redeploy doesn't blow away existing fields ([lifecycle-scripts.md](../../lifecycle-scripts.md)).
