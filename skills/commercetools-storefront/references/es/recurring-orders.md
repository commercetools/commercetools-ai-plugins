...
**Source:** Recurring Orders API Cheat Sheet (internal reference)
...
`intervalUnit` accepts `"Months"` or `"Days"`.
...
**Limitation (as of 2026-02-11):** Product Discounts cannot target recurrence policy-specific prices. Apply discounts via Cart Discounts instead.
...
To create a recurring order that starts in the future (Scenario 12 pattern):
...
Cart discounts now include a `recurringOrderScope` field that controls whether a discount applies to recurring order generations:
...
```json
{
  ...
  "updatedExpiresAt": "2026-12-31T00:00:00.000Z"
}
```

`updatedExpiresAt` is optional — use it to extend or reset the subscription expiry.
