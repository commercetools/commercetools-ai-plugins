...
The connector's inbound webhook (the `service` app) is tested at the router level — auth-rejection matrix, malformed-payload handling, duplicate-delivery idempotency — using the parent skill's [testing.md](../../testing.md). This file adds only the PIM-specific live-run layer.
...
- Use a **least-privilege** API client for the sandbox (`manage_products`, `manage_categories`, `manage_product_types` — not `manage_project`), so even a misfire is bounded → [security.md](../../security.md).
...
- [ ] Webhook `service` tested at the router level (auth matrix, malformed payload, duplicate delivery) → [testing.md](../../testing.md)
