...
For the deeper code-quality and security baseline (error hygiene that hides stack traces in production, structured logging that doesn't leak PII, the no-dead-code rule), the connector-build skill owns it: [`commercetools-connect` → security.md](../../../commercetools-connect/references/security.md) and [observability-operations.md](../../../commercetools-connect/references/observability-operations.md).
