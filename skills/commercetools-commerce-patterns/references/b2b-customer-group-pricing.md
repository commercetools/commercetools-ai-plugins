...
A customer can belong to **only one Customer Group at a time**. If a buyer's pricing tier changes (e.g. they move from "Silver" to "Gold"), the customer record must be updated to assign the new group, replacing the old one.
